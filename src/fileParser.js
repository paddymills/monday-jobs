...
// TODO: export class (service)

function fileHandler(fileText, state) {
  ...
  fileText
    ...
    .forEach(line => {
      ...
      switch (col) {
        case "Dates":
          jobs.updateJob(job, state.earlyStartColumn, line[2]);
          jobs.updateJob(job, state.mainStartColumn, line[3]);
          break;
        case "PM":
          jobs.updateJob(job, state.pmColumn, line[2], true);
          break;
        case "Products":
          jobs.updateJob(job, state.productsColumn, line[2]);
          break;
        case "Bays":
          if (COST_CENTERS.hasOwnProperty(line[2])) {
            jobs.updateJob(job, state.baysColumn, COST_CENTERS[line[2]]);
          }
          break;
        default:
...
export function fileParser(file, state) {
  file.text().then(res => fileHandler(res, state));
};